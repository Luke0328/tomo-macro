...
import { dateDataSchema } from "./db/DateData";
...
app.get('/api/dateData', authenticateToken, async (req, res) => {
	console.log("get dateData");
	await DbFacade.updateDateDate(req.body.user.email, [{date: new Date(), meals:[{name: "Meal 1", cals: 100, protein: 30, carbs: 50, fat: 20}], total_cals: 100, total_protein: 30, total_carbs: 50, total_fat: 20}])
	try {
		const user: any = await DbFacade.findUser(req.body.user.email);
...
app.post('/api/recipes', authenticateToken, async (req, res) => {
	...
	// console.log(req);
	try {
		await DbFacade.updateDateDate(req.body.user.email, req.body.dateData);
		res.status(201);
	} catch (e: any) {
		console.error(e);
		res.status(400);
